...
(二)功能介绍
    ...
    (5) 购买欢乐豆。
        ...
        (一.2)关于redis的问题
            ...
            7. 如何保证缓存和数据库的一致性  (缓存利用率 数据不一致)
                缓存和数据库的不一致性的原因可以从两个维度来看。第一个维度就是第一步成功，第二步失败的情况。第二个维度就是从并发的角度来看。
                第一个维度：
                1.先更新缓存再更新数据库。  如果更新数据库失败意味着当缓存失效之后读取的数据都是脏数据。
                2.先更新数据库再更新缓存。  更新缓存失败。当前读的式脏数据。
                这两个方案不仅是在一致性方面存在数据不一致的问题。
                数据发生变更的时候都更新缓存的话，但是缓存数据可能不经常访问，浪费缓存的资源。
                并且有时候还需要进行大量的计算再放到缓存浪费性能。
                ...
                4. 更新数据库删除缓存。(1)缓存中X不存在 (2)线程A读取数据库得到旧值x=1 (3)线程B更新缓存x=2 (4)线程B删除缓存 (5)线程A将旧值写入缓存x=1
                但是条件 1.缓存刚好失效 2.读请求+写请求并发 3.更新数据库+删除缓存时间 >读数据库+写缓存时间
                5. 延时双删策略。 在删除缓存更新数据库的基础上，防止3情况的发生，在修改数据库后等待一段时间重新删除缓存。
                缺点：保证了最终数据一致性，并不是强一致性。 但是需要消耗一点CPU资源。 一开始使用了当前线程阻塞并不好。
                应该交给一个延时任务去解决。
                6.如果redis挂了怎么处理。重试机制。可以添加一个消息队列来解决(ack机制 消息队列保证可靠性，保证消息的成功投递)。现在有开源项目专门监听binlog日志。放到消息队列中，然后消息队列消费消息去缓存中删除。
        (二)用户申请加入游戏
        (二.1)关于线程安全队列的选择
        ...
        (五.2)定时任务怎么选择的
            1.Timer和ScheduledThreadPoolExecutor原理几乎相同。我以定时线程为例子。
            从源码的角度来看主要式线程执行take()来获取任务。只有一个DelayQueue不同，本质上是一个小根堆数组。
            当任务添加到队列的时候计算还有多少时间执行。然后存放到小根堆中，保证了堆顶元素最先执行。
            一个for死循环。如果取出堆顶，判断delayTime已经<=0了那就立刻执行。然后执行完成后再放回队列中。
            如果delayTime>0 防止CPU的空转。添加了一把锁，调用await(dalayTime) 让线程进行阻塞。然后到达了这个时间就唤醒。
            2.使用了Springboot中的定时任务。springboot集成了定时任务的实现。
            添加@EnableScheduling 使用@Scheduing (里面存放了表达式)
        (五.3)定时分布式任务
            1. 如果使用其他第三方框架的话，Quartz集群分布式部署。 
            2. 如果自己实现的话，会考虑 数据库实现 redis实现
                (1)数据库。多个机器向数据库插入key value key要设置为unique约束。 能够插入的执行定时任务，插入失败的不能执行定时任务。
                (2)redis。用redis分布式锁来实现。获取到锁的那就执行，获取不到锁的不执行。设置过期时间防止死锁。
(三)秒杀
    整体的思路：(1)尽量将请求拦截到上游  (2)使用缓存减少直接对数据库的访问(三级缓存)  (3)使用rabbitmq进行异步和消峰
    (三.1)超卖问题 库存问题
    1.没有使用redis的情况。
        (1)可以使用synchronized代码块中实现扣减库存的操作。先获取stock 然后判断stock数量 然后再减去库存。
        (2)也可以使用 for update 显示加锁。
        (3)使用数据库的乐观锁，update stock=stock-1 where id=id and stock>0 不会超卖。
        在并发量高的场景下造成系统雪崩。容易出现多个请求同时竞争行锁。相互等待造成死锁。
    2.使用redis预减库存。
        (1)使用redis原生命令。扣减库存后判断库存是否>0 如果小于0说明失败。incrby(stock,-1)。但是回退库存的话导致不准确。
        (2)使用LUA脚本避免这个问题。 先获取库存然后判断库存是否>0 如果>0那就减库存返回库存数 如果小于等于0 直接返回0
    3.分布式锁解决 先从缓存中查询商品是否存在，如果不存在就去数据库中查找商品。 
        多个请求同时去查的话mysql有可能直接挂掉。
    (三.2)关于MQ异步处理  主要解耦合
    秒杀然后下单然后支付
    真正高流量和并发在秒杀。下单和支付实际并发量很小。所以设计秒杀系统的时候要把下单和支付从主流程中拆分。
    (三.3)MQ的应用场景
        1. 异步处理。用户注册，写入数据库，发送邮件和短信。  写入数据库后写到消息队列。然后异步处理。减少响应时间。
        2. 应用解耦合。两个服务器之间直接调用，耦合。如果下线的话，那么就导致处理失败。添加到消息队列中解耦合。
        3. 流量削峰。大流量的情况下，将并发任务封装成消息然后放到MQ中，如果超过最大数量就拒绝服务，跳转错误页面。
        4. 消息的订阅和发布。
    (三.4)引入消息队列产生的问题
        1.消息丢失的情况。
            (1)消息在传入消息队列的时候丢失。
            开启rabbitMQ的confirm机制。生产者投递消息后，mq接收到消息会回复生产者一个应答消息。生产者能够收到应答信号。
            channel.confirmSelect()  channle.addConfirmListen()在channel上添加监听，监听mq返回的应答。
            (2)RabbitMQ收到消息存到内存中，自己挂掉，消息丢失。
            开启rabbitMQ的持久化机制。将mq中的消息写到了磁盘中。mq挂了重启之后又能加载数据。
            结合confirm机制。持久化之后才会通知生产者ack。
            (3)消费者收到消息但是没来得及处理就挂了，RabbitMQ以为消息处理了。
            rabbitmq提供的ack机制。关闭自动的ack机制。在消费者处理完消息之后，再次在程序里面ack一次。
        1.消息丢失问题。
            秒杀成功后向MQ发送下单消息有可能失败。网络弯头或者MQ服务器磁盘问题。
            解决方法：添加一张消息发送表。记录这个消息的状态。当mq消费了之后再来修改这个消息的状态。然后删除。
            如果MQ没有收到消息，那就固定时间重试。
        2.重复消费问题
            ack应答机制。
            添加一张消费表。每个消息添加一个UUID。没消费一条数据那就从消费表中找这个UUID。如果有那就不操作。
            如果没有的话，那就写道消费表然后添加订单。
    (三.5)延迟消费问题  如果15min没有支付那就取消。
        1. 如果生成订单后，发送mq消息，添加到延迟队列中(15min) ，然后消费消息。这个时候判断订单的状态。
        如果没有支付那就取消订单。如果已经支付那就返回。
        2.用户支付完成后就会修改订单状态为已经支付。
        3.或者使用jdk的延时队列/定时任务解决。
    (三.6)MQ的几种模型
        1. 单生产者--->队列--->单消费者。
        2. 单生产者--->队列---> 多消费者模型。多个消费者共同消费队列中的消息。
        3. fanout模型。生产者--->交换机 ---->多个队列 --->消费者绑定队列。 
        4. direct模型。routingKey和队列进行绑定。交换机会根据消息的类型将其发送到对应的队列中。
        5. topic模型。routingKey进行模糊匹配。 #匹配一个或者多个单词  *只能匹配一个单词
...
(四)亮点难点
    ...
    详解
    (三.2.1)IO的优化。
        1.使用SocketBIO的时候的QPS和并发量。


        
    (三.4.2)心跳检查
        因为TCP是长连接并且网络是不可靠的，有些突发情况回导致服务器和客户端的连接中断。但是这个时候恰好服务器和客户端没有通信，短时间内不能发现对方掉线。并且占用了fd。造成资源浪费。
        引入了心跳机制。两种实现。第一个是TCP层面的心跳检测。keepalive机制。缺点：默认是关闭的，而且依赖操作系统，心跳检查时间两个小时。
        第二个是应用层面。Netty对心跳检查支持。
        在pipeline中添加心跳检查Handler。重写Handler中的方法。然后监听事件。给读写事件设置事件。多少时间触发事件。
        检查的方法有很多，我使用了一个简单的方法。在客户端是 5s钟触发一个读写事件，然后向服务端发送消息。Ping。
        服务端收到ping消息后立刻恢复pong消息给客户端。 那么服务器11s钟触发一个读事件。如果客户端在两个该发送的消息都没有到达
        服务端的话，可以认为网络产生了异常。服务端主动关闭连接，然后触发了客户端的Inavtive方法。然后可以选择重连或者是关闭连接。
    (三.4.3)断线重连
    ...
    (三.4.5)粘包拆包的处理
        1. Nage算法。
            Nagle算法主要是解决TCP数据报中的实际发送的字节数很少的问题，Nagle算法将很多小的数据包保存起来，然后使用一个TCP报文段发送。
            一来解决网络中的数据包数量过多，而来减少网络的带宽。  那么这样就造成了数据发送的时候粘包的问题。
        2. 为什么会产生粘包拆包的问题
            (1)谈到粘包拆包问题首先想说Nagle算法。
            (2)本质上并不是Nagle的锅，我认为应该从用户缓冲区和Socket缓冲区来说明。
                IP数据报的最大字节为1500，从运输层面来看，只能存放1460个字节的用户数据。IP数据报头部20字节，TCP头部20字节。
                TCP协议保证了每个TCP报文段能够安全可靠并且有序的到达。但是TCP并不能感知用户的消息。
                当用户缓冲区和Socket缓冲区关系的时候考虑。
        3. 解决方案
            (1)每个包固定长度。
            (2)在包的尾部添加分隔符。
            (3)先发送字节数组的长度然后在发送字节码　　读数据的时候先读包长度然后读取字节数组转成对象，添加到ｌｉｓｔ中。
            我使用了protobuf的编解码器来解决。本质上也是发送字节数组的长度然后发送字节数组来实现。
            解码器的时候要注意说明解码的对象的类型是说明，因为字节数组要转化成对象。
