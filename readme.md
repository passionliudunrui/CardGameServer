...
(三)亮点难点
    ...
    (三.2)如何提升项目并发量
        ...
        1. IO模型的优化。 BIO-->NIO--->Netty  sql语句的优化
        2. 架构的优化。 多线程+BIO  线程池+BIO  NIO   Netty  Netty+动态业务线程池
        3. 引入缓存层。
    (三.3)如何保证用户的信息安全
    ...
    (三.4)如何提升游戏体验
        ...
        2. 提供心跳检查机制，进行长时间离开退出游戏给出友好提示。或者服务器宕机给出友好提醒。
        3. 提供断线重连机制。因为网络中断后重新连接游戏。
...
(五)代办
    ...
    4.布隆过滤器，随机过期时间。
    5.分布式的场景下，怎么防止超卖？
    6.mybatis分页  大概说了一下分页插件和自己写分页这样
